...
化粧品BtoC個人販売者向けの販売管理Webアプリ（v3.5）。月末の手書き伝票から7種類の書類への転記を自動化する。
...
PHPファイルの更新は rsync で個別アップロード（`npm run deploy` では api/ ディレクトリが除外される）:
```bash
rsync -avz server/api/customers.php xserver-blog:/home/evahpro/evahpro.xsrv.jp/public_html/arsoa/api/
```
...
- サーバー版: `src/utils/api.js` の `ApiClient` クラス（`/arsoa/api/` のPHP APIへfetch）。401レスポンスで自動ログアウト。
...
### レスポンシブ対応
`src/styles/breakpoints.js` でブレークポイント定義。`src/hooks/useMediaQuery.js` が `isMobile` / `isTablet` / `isDesktop` を返す。

```javascript
// breakpoints.js
{ mobile: 480, tablet: 1024, desktop: 1280, wide: 1440 }
```

**tablet: 1024** — iPad横持ち（1024px）をモバイルUIに含めるため。`isMobile` は幅1024px以下で `true`。タブレット用UIは `isMobile && !isTablet` ではなく `isMobile` で判定。

モバイル専用コンポーネントは `src/components/mobile/` に分離:
- `MobileProductSelector.jsx` — 商品選択UI
- `MobileSalesHistory.jsx` — 売上履歴UI

### OCR機能
`src/services/ocrService.js` — Google Cloud Vision APIキー管理と画像テキスト認識。  
`src/services/slipParser.js` — OCRテキストから商品コード・カテゴリへのパース（手書き略語辞書を内包）。
...
```javascript
{
  id: 1234567890,       // タイムスタンプベースID
  name: '田中花子',
  yomi: 'たなかはなこ', // 読み仮名（あいうえお順ソートに使用）
  rank: 'C',            // A/B/C/D
  isAppUser: false      // 販売者自身の自己使用フラグ
}
```

**yomiフィールドの注意**:
- `storage.js` の `loadAllData()` で後方互換処理: `'yomi' in c ? c.yomi : c.name`（空文字は空文字のまま保持）
- `server/api/customers.php` の SELECT は `ORDER BY CASE WHEN yomi = '' THEN name ELSE yomi END, name` でyomi順を保証
- 新規登録時にyomiが空だと並べ替えがname（漢字Unicode順）になる
...
書類6の⑧経費合計・⑩在庫金額は**月末レポート画面で入力**し、収支計算書は読み取り専用で表示する。収支計算書の売上高はsalesデータからリアルタイム計算（monthlyReportsの値は使わない）。
